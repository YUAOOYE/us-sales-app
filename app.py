...
# ==============================================================================
st.set_page_config(
    page_title="北美大零售全品类与气候销售决策系统 (Grand Unified Master)",
    page_icon="🏬",
    layout="wide",
...
with st.sidebar:
    ...
    fob_cost = st.number_input("1. 出厂供货价 FOB ($/件)：", min_value=0.5, max_value=200.0, value=3.80, step=0.2)
    ocean_freight = st.number_input("2. 单件预估海运费 ($/件)：", min_value=0.0, max_value=50.0, value=0.65, step=0.05)
    ...
    st.markdown(f"• 到岸完税成本 (Landed): **${landed_cost:.2f}**")
    st.markdown(f"• 商超单件毛利: **${gross_profit_unit:.2f}**")
    
    if buyer_margin < 40.0:
        st.markdown(f'<span class="badge-tier4">⚠️ 零售到岸毛利率：{buyer_margin:.1f}% (偏低，低于40%门槛)</span>', unsafe_allow_html=True)
    elif buyer_margin <= 55.0:
        st.markdown(f'<span class="badge-tier1">✅ 零售到岸毛利率：{buyer_margin:.1f}% (黄金健康区间)</span>', unsafe_allow_html=True)
    else:
    ...
    with st.expander("2. ASHRAE 暖通与 HDD/CDD 能耗度日", expanded=False):
        st.write("""
        * **HDD (采暖度日)**：> 5000 区域长年开暖气，出风口长年承受 55℃~65℃ 干燥热风烘烤，防热变形是硬指标。
        * **CDD (制冷度日)**：> 2000 区域长夏高湿开冷气，天花散流器防结露冷凝滴水是第一大痛点。
        """)
    with st.expander("3. Frost Line 冻土深度与超长水阀", expanded=False):
        st.write("""
        北方冻土深达 36~60 英寸。去 Home Depot 买室外防冻龙头必须选 **8~12 英寸超长杆**，才能把阀体送入室内保温区；南方买 4 英寸即可。
        """)
    with st.expander("4. 融雪盐腐蚀与材质选用", expanded=False):
        st.write("""
        降雪带在路面撒巨量化冰盐。鞋底盐水带入玄关，冷轧钢风口与压条 2 年即生锈烂穿。当地必须推**阳极氧化铝（6063）**或 304 不锈钢。
        """)
    ...
    with st.expander("7. Defective Allowance 残损扣款", expanded=False):
        st.write("""
        大商超合同常规定期在发票扣取 **2% ~ 4%** 作为无理由退货与残损备用金。报价核算时必须计入此项成本。
        """)
...
STATES_DATA = {
    "NC": {
        ...
        "best": "地面出风口、实木高低压条、防潮五金", "avoid": "未做防锈处理冷轧薄铁件",
    ...
    "TN": {
        ...
        "best": "重载地面风口、承重踏压五金", "avoid": "天花专用下送风散流器",
    ...
    "KY": {
        ...
        "best": "地下管道风口、复古铸铁件", "avoid": "无调节阀轻薄空框",
    ...
    "SC": {
        ...
        "best": "工程ABS风口、耐盐雾构件", "avoid": "未做防腐普通冷轧铁件",
    ...
    "WV": {
        "cn": "西弗吉尼亚州", "name": "West Virginia", "region": "美东", "thd": 10, "lowes": 17, "menards": 1, "dc": "匹兹堡分仓",
        "foundation": "山地深地基/全地下室", "climate": "Zone 5A 湿润山地极寒", "flooring": "LVP 40%, 实木 30%, 瓷砖 20%, 地毯 10%",
        "best": "抗重压地面风口、防冻五金", "avoid": "低温脆性塑料件",
        "size_breakdown": "标准通用型: 4x10 (70%), 4x12 (15%), 2x12 (10%), 6x12 (5%)",
        "channel_advice": "传统耐用型五金主场，价格敏感度较高，主打坚固承重耐踩。",
        "hdd": 5000, "cdd": 950, "frost_depth": 30, "water_hardness": 7.0, "salt_risk": "高", "hazard": "山地积雪重融雪盐", "house_age": 52, "hazard_idx": 5, "base_vel": 34.0
    },
    "VA": {
        "cn": "弗吉尼亚州", "name": "Virginia", "region": "美东", "thd": 50, "lowes": 61, "menards": 0, "dc": "里士满仓",
        "foundation": "地下室/架空层老房多", "climate": "Zone 4A 四季湿润冬冷", "flooring": "实木 45%, LVP 35%, 瓷砖 10%, 地毯 10%",
        "best": "中高端装饰风口、精工五金", "avoid": "粗糙低端工程塑料件",
        "size_breakdown": "地面主力: 4x10 (65%); 4x12 (20%); 2x12 (10%); 回风: 6x10 (5%)",
        "channel_advice": "居民消费水准高，愿意为仿古铜、拉丝镍等精工颜值支付 30%+ 溢价。",
        "hdd": 3900, "cdd": 1450, "frost_depth": 18, "water_hardness": 5.0, "salt_risk": "高", "hazard": "沿海湿润冻融交替", "house_age": 42, "hazard_idx": 4, "base_vel": 33.5
    },
    "OH": {
        ...
        "best": "冲压金属/铝合金地面风口", "avoid": "薄脆塑料、天花专用下吹风口",
        "size_breakdown": "地面主力: 4x10 (65%); 4x12 (20%); 2x12 (10%); 回风: 6x10 (5%)",
    ...
    "IL": {
        ...
        "best": "重型地面出风口、阳极铝压条", "avoid": "轻薄无卡扣风口",
        "size_breakdown": "地面主力: 4x10 (65%); 4x12 (20%); 2x12 (10%); 回风: 6x10 (5%)",
    ...
    "MI": {
        ...
        "best": "经典地面金属风口、耐雪盐五金", "avoid": "天花风口、薄铁喷漆",
        "size_breakdown": "地面主力: 4x10 (65%); 4x12 (20%); 2x12 (10%); 回风: 6x10 (5%)",
    ...
    "PA": {
        "cn": "宾夕法尼亚州", "name": "Pennsylvania", "region": "美东", "thd": 73, "lowes": 83, "menards": 0, "dc": "阿伦敦/费城",
        "foundation": "老房地下室比例极高", "climate": "Zone 5A/6A 寒冷多雪老区", "flooring": "实木 45%, LVP 35%, 瓷砖 10%, 地毯 10%",
        "best": "复古雕花金属风口、铸铁件", "avoid": "现代极简无框塑料",
        "size_breakdown": "地面主力: 4x10 (65%); 4x12 (20%); 2x12 (10%); 回风: 6x10 (5%)",
        "channel_advice": "注重传统复古美学，旧房木地板配金属或铸铜风口是绝对主流。",
        "hdd": 5400, "cdd": 900, "frost_depth": 38, "water_hardness": 8.5, "salt_risk": "极高", "hazard": "阿巴拉契亚冻融融雪盐", "house_age": 58, "hazard_idx": 7, "base_vel": 28.5
    },
    "MO": {
        "cn": "密苏里州", "name": "Missouri", "region": "美中", "thd": 39, "lowes": 33, "menards": 19, "dc": "圣路易斯",
        "foundation": "传统全地下室木屋", "climate": "Zone 4A/5A 大陆季风冬冷", "flooring": "LVP 40%, 实木 30%, 瓷砖 20%, 地毯 10%",
        "best": "地面可调风口、管件辅料", "avoid": "纯热带建材",
        "size_breakdown": "标准通用型: 4x10 (70%), 4x12 (15%), 2x12 (10%), 6x12 (5%)",
        "channel_advice": "圣路易斯与堪萨斯城都市圈带动全州动销，标准化 4x10 通货畅销。",
        "hdd": 4800, "cdd": 1400, "frost_depth": 28, "water_hardness": 11.0, "salt_risk": "高", "hazard": "剧烈温差冰融循环", "house_age": 47, "hazard_idx": 5, "base_vel": 27.5
    },
    "GA": {
        "cn": "乔治亚州", "name": "Georgia", "region": "美东南", "thd": 91, "lowes": 65, "menards": 0, "dc": "亚特兰大(THD全球总部)",
        "foundation": "北架空层/南水泥平板", "climate": "Zone 3A 亚热带长夏湿热", "flooring": "LVP 40%, 瓷砖 35%, 地毯 25%",
        "best": "天花与地面品类分推、防腐件", "avoid": "全推纯地面件",
        "size_breakdown": "地面与天花混合: 4x10 (40%), 散流器 (40%), 墙面滤网 (20%)",
        "channel_advice": "The Home Depot 全球总部所在地，标杆渠道，买手会重点审视此州门店动销。",
        "hdd": 2700, "cdd": 1850, "frost_depth": 5, "water_hardness": 3.5, "salt_risk": "低", "hazard": "长夏湿热白蚁雷暴", "house_age": 31, "hazard_idx": 5, "base_vel": 27.0
    },
    "NY": {
        "cn": "纽约州", "name": "New York", "region": "美东北", "thd": 100, "lowes": 68, "menards": 0, "dc": "奥尔巴尼/水牛城",
        "foundation": "市区公寓无风管/郊区独栋地下室", "climate": "Zone 5A/6A 湿冷上州大雪", "flooring": "实木 45%, LVP 35%, 瓷砖 10%, 地毯 10%",
        "best": "郊区独栋地面件、复古暖气罩", "avoid": "对市区公寓推地板风管件",
        "size_breakdown": "地面主力: 4x10 (65%); 4x12 (20%); 2x12 (10%); 回风: 6x10 (5%)",
        "channel_advice": "纽约市区与上州完全割裂，需按店分发，市区店少铺地板出风口。",
        "hdd": 5900, "cdd": 800, "frost_depth": 48, "water_hardness": 5.5, "salt_risk": "极高", "hazard": "水牛城湖效应暴雪+老房改建", "house_age": 62, "hazard_idx": 7, "base_vel": 26.0
    },
    "WI": {
        "cn": "威斯康星州", "name": "Wisconsin", "region": "美中", "thd": 28, "lowes": 14, "menards": 45, "dc": "欧克莱尔(Menards大本营)",
        "foundation": "全地下室普及率极高", "climate": "Zone 5A/6A 漫长严寒多雪", "flooring": "实木 45%, LVP 35%, 瓷砖 10%, 地毯 10%",
        "best": "重型金属地板出风口、防冻五金", "avoid": "天花专用风口",
        "size_breakdown": "地面主力: 4x10 (65%); 4x12 (20%); 2x12 (10%); 回风: 6x10 (5%)",
        "channel_advice": "Menards 核心发源腹地，本土品牌极强，供货需打出极高性价比与耐用度。",
        "hdd": 7400, "cdd": 550, "frost_depth": 54, "water_hardness": 14.5, "salt_risk": "极高", "hazard": "漫长严冬暴雪强融雪盐", "house_age": 51, "hazard_idx": 7, "base_vel": 25.5
    },
    "MN": {
        "cn": "明尼苏达州", "name": "Minnesota", "region": "美中", "thd": 35, "lowes": 12, "menards": 38, "dc": "明尼阿波利斯",
        "foundation": "深层地下室但水暖多", "climate": "Zone 6A/7 极寒雪原", "flooring": "实木 45%, LVP 35%, 瓷砖 10%, 地毯 10%",
        "best": "防冷凝风口、超长水阀(12寸)", "avoid": "易脆塑料、短水阀",
        "size_breakdown": "地面主力: 4x10 (65%); 4x12 (20%); 2x12 (10%); 回风: 6x10 (5%)",
        "channel_advice": "冻土深达 60 寸，室外龙头采购只认 10-12 寸以上长杆防冻自泄水阀。",
        "hdd": 8500, "cdd": 550, "frost_depth": 60, "water_hardness": 14.0, "salt_risk": "极高", "hazard": "全美最深冻土(60寸)极寒", "house_age": 46, "hazard_idx": 8, "base_vel": 24.5
    },
    "CO": {
        "cn": "科罗拉多州", "name": "Colorado", "region": "山地大区", "thd": 47, "lowes": 28, "menards": 0, "dc": "丹佛物流仓",
        "foundation": "防冻全地下室", "climate": "Zone 5B/6B 高海拔强积雪", "flooring": "LVP 40%, 实木 30%, 瓷砖 20%, 地毯 10%",
        "best": "气密保温风口、实木嵌入风口", "avoid": "漏风劣质薄件",
        "size_breakdown": "标准通用型: 4x10 (70%), 4x12 (15%), 2x12 (10%), 6x12 (5%)",
        "channel_advice": "绿色建材与高气密标准深入人心，注重 WUI 山火防飞烬认证。",
        "hdd": 6200, "cdd": 600, "frost_depth": 36, "water_hardness": 7.5, "salt_risk": "高", "hazard": "WUI山火防飞烬/强紫外线", "house_age": 38, "hazard_idx": 5, "base_vel": 24.0
    },
    "TX": {
        ...
        "best": "天花散流器、回风百叶格栅", "avoid": "地面下沉式出风口(无孔可用!)",
    ...
    "FL": {
        ...
        "best": "天花散流器、铝/ABS防腐地漏", "avoid": "地面出风口(绝对禁发货!)",
    ...
    "CA": {
        ...
        "foundation": "南加全平板/网点极密", "climate": "Zone 3B/4B 干燥温和干热", "flooring": "瓷砖 45%, LVP 35%, 地毯 20%",
        "best": "Title 24环保风口、WUI防飞烬网", "avoid": "高铅铸造件、普通易燃塑料",
        "size_breakdown": "天花主推: 6x6, 8x8, 10x10 (60%); 侧墙格栅: 14x6 (40%)",
        "channel_advice": "全美最大单一经济体，但加州 65 号提案（Prop 65 铅标）是法律诉讼雷区，必须贴标。",
        "hdd": 2100, "cdd": 1300, "frost_depth": 0, "water_hardness": 9.5, "salt_risk": "极低", "hazard": "WUI山火/地震/Prop 65铅标", "house_age": 48, "hazard_idx": 7, "base_vel": 20.0
    },
    "WA": {
        "cn": "华盛顿州", "name": "Washington", "region": "美西北", "thd": 48, "lowes": 38, "menards": 0, "dc": "西雅图枢纽",
        "foundation": "架空层与现代高气密住宅", "climate": "Zone 4C 阴湿多雾", "flooring": "LVP 40%, 实木 30%, 瓷砖 20%, 地毯 10%",
        "best": "极简线形风口、哑光黑五金", "avoid": "过时粗糙件、易生锈铁",
        "size_breakdown": "标准通用型: 4x10 (70%), 4x12 (15%), 2x12 (10%), 6x12 (5%)",
        "channel_advice": "西雅图高科技中产多，偏好黑白灰极简现代风，环保无味认证受欢迎。",
        "hdd": 4900, "cdd": 300, "frost_depth": 18, "water_hardness": 2.5, "salt_risk": "低", "hazard": "长年阴雨高湿室内霉菌", "house_age": 43, "hazard_idx": 4, "base_vel": 23.5
    },
    "AZ": {
        "cn": "亚利桑那州", "name": "Arizona", "region": "美西南", "thd": 58, "lowes": 33, "menards": 0, "dc": "凤凰城",
        "foundation": "绝大多数为混凝土平板", "climate": "Zone 2B 纯沙漠干热长酷暑", "flooring": "瓷砖 50%, 强化地板 30%, 地毯 20%",
        "best": "天花散流器、遮阳与抗晒五金", "avoid": "地面出风口",
        "size_breakdown": "天花散流器: 6x6, 8x8, 12x12 (75%); 侧墙回风 (25%)",
        "channel_advice": "凤凰城全面以顶装风口为主，强光紫外线耐受与防脆化是测试红线。",
        "hdd": 1200, "cdd": 3500, "frost_depth": 0, "water_hardness": 16.5, "salt_risk": "极低", "hazard": "全美最高CDD沙漠酷暑+极硬水", "house_age": 30, "hazard_idx": 3, "base_vel": 19.5
    },
    "AK": {
        "cn": "阿拉斯加州", "name": "Alaska", "region": "美西北", "thd": 7, "lowes": 5, "menards": 0, "dc": "安克雷奇驳运仓",
        "foundation": "永久冻土抬升/深基保温仓", "climate": "Zone 7/8 全美最高寒", "flooring": "强化保温 50%, 实木 30%, 瓷砖 20%",
        "best": "超耐极温五金、重载保温出风件", "avoid": "常温薄脆塑料及冷轧薄铁",
        "size_breakdown": "重型金属: 4x10 (60%), 4x12 (25%), 6x12 (15%)",
        "channel_advice": "海运补货前置期长达 7-10 天，单次起订量需以托盘整柜推进，规避零散补货。",
        "hdd": 10500, "cdd": 0, "frost_depth": 72, "water_hardness": 6.0, "salt_risk": "中", "hazard": "永久冻土层+HDD突破10,000", "house_age": 42, "hazard_idx": 8, "base_vel": 25.0
    },
    "HI": {
        "cn": "夏威夷州", "name": "Hawaii", "region": "美西", "thd": 7, "lowes": 4, "menards": 0, "dc": "火奴鲁鲁海运仓",
        "foundation": "火山岩/架空防潮桩", "climate": "Zone 1 强热带高盐雾高腐蚀", "flooring": "耐水瓷砖 60%, 防水LVP 30%, 竹木 10%",
        "best": "316/304不锈钢、纯ABS防腐件", "avoid": "含铁电镀件、地面出风口",
        "size_breakdown": "天花抗盐雾格栅: 6x6, 8x8, 12x12",
        "channel_advice": "纯海岛极端高盐雾，普通电镀件 3 个月即生锈，必须推纯不锈钢与工程塑料。",
        "hdd": 0, "cdd": 4200, "frost_depth": 0, "water_hardness": 3.0, "salt_risk": "低", "hazard": "纯海岛极端高盐雾高氧化", "house_age": 47, "hazard_idx": 6, "base_vel": 18.0
    }
}
...
EXTRA_STATES = {
    ...
    "CT": {"cn": "康涅狄格州", "name": "Connecticut", "reg": "美东北", "thd": 30, "low": 16, "men": 0, "dc": "哈特福德", "hdd": 5800, "cdd": 750, "fd": 42, "wh": 4.0, "sr": "极高", "haz": "老旧改建", "age": 59, "v": 26.0},
    "DE": {"cn": "特拉华州", "name": "Delaware", "reg": "美东", "thd": 9, "low": 6, "men": 0, "dc": "费城", "hdd": 4600, "cdd": 1200, "fd": 24, "wh": 5.5, "sr": "高", "haz": "海盐", "age": 41, "v": 27.5},
    "ID": {"cn": "爱达荷州", "name": "Idaho", "reg": "美西北", "thd": 13, "low": 8, "men": 0, "dc": "盐湖城", "hdd": 6800, "cdd": 550, "fd": 36, "wh": 8.0, "sr": "高", "haz": "冻融", "age": 33, "v": 29.0},
    "IA": {"cn": "爱荷华州", "name": "Iowa", "reg": "美中", "thd": 16, "low": 15, "men": 22, "dc": "得梅因", "hdd": 6700, "cdd": 850, "fd": 48, "wh": 16.0, "sr": "极高", "haz": "雪盐", "age": 54, "v": 28.0},
    "KS": {"cn": "堪萨斯州", "name": "Kansas", "reg": "美中", "thd": 18, "low": 14, "men": 9, "dc": "堪萨斯城", "hdd": 5000, "cdd": 1350, "fd": 30, "wh": 13.5, "sr": "高", "haz": "狂风", "age": 47, "v": 28.5},
    "LA": {"cn": "路易斯安那州", "name": "Louisiana", "reg": "美南", "thd": 28, "low": 30, "men": 0, "dc": "新奥尔良", "hdd": 1600, "cdd": 2600, "fd": 0, "wh": 4.5, "sr": "极低", "haz": "飓风湿热", "age": 41, "v": 21.5},
    "ME": {"cn": "缅因州", "name": "Maine", "reg": "美东北", "thd": 11, "low": 10, "men": 0, "dc": "波特兰", "hdd": 7800, "cdd": 300, "fd": 54, "wh": 3.0, "sr": "极高", "haz": "海雾严冬", "age": 53, "v": 25.0},
    "MD": {"cn": "马里兰州", "name": "Maryland", "reg": "美东", "thd": 43, "low": 30, "men": 0, "dc": "巴尔的摩", "hdd": 4500, "cdd": 1300, "fd": 24, "wh": 6.5, "sr": "高", "haz": "湾区潮湿", "age": 46, "v": 28.0},
    "MA": {"cn": "马萨诸塞州", "name": "Massachusetts", "reg": "美东北", "thd": 45, "low": 29, "men": 0, "dc": "波士顿", "hdd": 5900, "cdd": 700, "fd": 48, "wh": 3.5, "sr": "极高", "haz": "老房水暖", "age": 60, "v": 26.5},
    "MS": {"cn": "密西西比州", "name": "Mississippi", "reg": "美南", "thd": 16, "low": 22, "men": 0, "dc": "杰克逊", "hdd": 2300, "cdd": 2100, "fd": 0, "wh": 3.5, "sr": "极低", "haz": "高湿热", "age": 39, "v": 22.0},
    "MT": {"cn": "蒙大拿州", "name": "Montana", "reg": "美西北", "thd": 8, "low": 5, "men": 0, "dc": "比灵斯", "hdd": 7900, "cdd": 400, "fd": 54, "wh": 9.0, "sr": "高", "haz": "深冻土", "age": 45, "v": 25.5},
    "NE": {"cn": "内布拉斯加州", "name": "Nebraska", "reg": "美中", "thd": 11, "low": 7, "men": 10, "dc": "奥马哈", "hdd": 6200, "cdd": 950, "fd": 42, "wh": 13.5, "sr": "极高", "haz": "雪蚀", "age": 49, "v": 26.5},
    "NV": {"cn": "内华达州", "name": "Nevada", "reg": "美西", "thd": 21, "low": 17, "men": 0, "dc": "拉斯维加斯", "hdd": 2800, "cdd": 2600, "fd": 12, "wh": 18.0, "sr": "极低", "haz": "硬水高温", "age": 28, "v": 20.5},
    "NH": {"cn": "新罕布什尔州", "name": "New Hampshire", "reg": "美东北", "thd": 17, "low": 11, "men": 0, "dc": "曼彻斯特", "hdd": 7100, "cdd": 450, "fd": 50, "wh": 2.5, "sr": "极高", "haz": "林区寒冬", "age": 48, "v": 25.5},
    "NJ": {"cn": "新泽西州", "name": "New Jersey", "reg": "美东北", "thd": 65, "low": 40, "men": 0, "dc": "纽瓦克", "hdd": 4900, "cdd": 1200, "fd": 30, "wh": 6.5, "sr": "极高", "haz": "化冰盐腐蚀", "age": 57, "v": 27.5},
    "NM": {"cn": "新墨西哥州", "name": "New Mexico", "reg": "美西南", "thd": 17, "low": 13, "men": 0, "dc": "阿尔伯克基", "hdd": 3800, "cdd": 1500, "fd": 18, "wh": 13.0, "sr": "低", "haz": "风沙硬水", "age": 36, "v": 22.5},
    "ND": {"cn": "北达科他州", "name": "North Dakota", "reg": "美中", "thd": 4, "low": 3, "men": 8, "dc": "法戈", "hdd": 9400, "cdd": 450, "fd": 66, "wh": 13.0, "sr": "极高", "haz": "最高HDD极寒", "age": 45, "v": 24.5},
    "OK": {"cn": "俄克拉荷马州", "name": "Oklahoma", "reg": "美南", "thd": 21, "low": 24, "men": 0, "dc": "达拉斯", "hdd": 3400, "cdd": 1900, "fd": 15, "wh": 11.5, "sr": "中", "haz": "龙卷风", "age": 40, "v": 26.0},
    "OR": {"cn": "俄勒冈州", "name": "Oregon", "reg": "美西北", "thd": 27, "low": 18, "men": 0, "dc": "波特兰", "hdd": 4600, "cdd": 400, "fd": 12, "wh": 2.0, "sr": "低", "haz": "长年阴雨", "age": 43, "v": 26.5},
    "RI": {"cn": "罗德岛州", "name": "Rhode Island", "reg": "美东北", "thd": 7, "low": 5, "men": 0, "dc": "普罗维登斯", "hdd": 5600, "cdd": 750, "fd": 40, "wh": 3.5, "sr": "极高", "haz": "海盐雪蚀", "age": 60, "v": 25.0},
    "SD": {"cn": "南达科他州", "name": "South Dakota", "reg": "美中", "thd": 3, "low": 3, "men": 5, "dc": "苏瀑", "hdd": 7600, "cdd": 700, "fd": 54, "wh": 16.5, "sr": "极高", "haz": "深冻土", "age": 47, "v": 25.0},
    "UT": {"cn": "犹他州", "name": "Utah", "reg": "山地大区", "thd": 24, "low": 14, "men": 0, "dc": "盐湖城", "hdd": 5800, "cdd": 1050, "fd": 30, "wh": 17.5, "sr": "高", "haz": "大温差硬水", "age": 32, "v": 25.5},
    "VT": {"cn": "佛蒙特州", "name": "Vermont", "reg": "美东北", "thd": 5, "low": 4, "men": 0, "dc": "伯灵顿", "hdd": 7600, "cdd": 350, "fd": 54, "wh": 4.5, "sr": "极高", "haz": "漫长积雪", "age": 51, "v": 24.5},
    "WY": {"cn": "怀俄明州", "name": "Wyoming", "reg": "山地大区", "thd": 5, "low": 3, "men": 2, "dc": "夏延", "hdd": 7500, "cdd": 350, "fd": 50, "wh": 11.0, "sr": "高", "haz": "极高寒", "age": 42, "v": 23.5}
}
...
for k, v in EXTRA_STATES.items():
    if k not in STATES_DATA:
        STATES_DATA[k] = {
            ...
            "dc": v["dc"], "foundation": "全地下室/标准木结构", "climate": "Zone 4A/5A 大陆性气候", "flooring": "实木 40%, LVP 35%, 瓷砖 15%, 地毯 10%",
            "best": "常规标准修缮件", "avoid": "非标冷门异形件",
            "size_breakdown": "标准通用型: 4x10 (70%), 4x12 (15%), 2x12 (10%), 6x12 (5%)",
            "channel_advice": "标准流通市场，按大区干线物流常规集装箱整托补货即可。",
...
col_cat, col_region, col_season, col_channel = st.columns([1.2, 0.9, 1.0, 1.1])
...
with f1:
    ...
    sel_pos_raw = st.selectbox("1. 安装位置 (Position)：", pos_opts, index=1)
    sel_pos = sel_pos_raw.split(" ")[0]
with f2:
    mat_opts = ["(全部材质 / All)"] + cat_cfg["materials"]
    sel_mat_raw = st.selectbox("2. 生产原材料 (Material)：", mat_opts, index=0)
    sel_mat = sel_mat_raw.split(" ")[0]
with f3:
    fin_opts = ["(全部颜色 / All)"] + cat_cfg["finishes"]
    sel_fin_raw = st.selectbox("3. 表面工艺/颜色 (Finish)：", fin_opts, index=0)
    sel_fin = sel_fin_raw.split(" ")[0]
with f4:
    size_opts = ["(全部尺寸 / All)"] + cat_cfg["sizes"]
    sel_size_raw = st.selectbox("4. 规格尺寸 (Size)：", size_opts, index=1)
...
for abbr, s in STATES_DATA.items():
    ...
    if s["salt_risk"] in ["高", "极高"]:
        ...
        elif sel_mat in ["Aluminum", "Stainless", "304"]:
            weight *= 1.25
    ...
    if calc_vel >= 38.0:
        ...
        tier_str = "Tier 2 (A级主力)"
    elif calc_vel >= 14.0:
        tier_str = "Tier 3 (B级走量)"
    else:
...
with k2:
    st.markdown(f"""
    <div class="kpi-card">
        <div class="kpi-title">当前筛选·月度总销量预估</div>
...
with tab_rank:
    view_mode = st.radio(
        "切换主排序依据：", 
        ["全州渠道总吞吐量 (件/月) - 适于工厂备货与海运整柜排产", "单店平均销售流速 (件/店/月) - 适于商超平效谈判与货架位申请"], 
        horizontal=True
    )
...
csv_out = df_sorted[[
    ...
    "salt_risk", "house_age", "hazard", "reason_desc"
]].to_csv(index=False).encode('utf-8-sig')

st.download_button(
    label=f"📥 导出【{cat_cfg['name'].split(' ')[0]}】({selected_region}) 完整商业决策模型报表 (.csv)",
    data=csv_out,
    file_name=f"NA_Retail_Master_Decision_{selected_cat_key}_{selected_region[:3]}.csv",
    mime="text/csv"
)
